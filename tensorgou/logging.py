...
class Logging(object):
    ...
    def message(message):
        ...
        log_print("{}: {}".format(time.strftime("%Y-%m-%d %H:%M:%S"), message))

    @staticmethod
    def warnning(message):
        """Logs message with a colored timestamp."""
        log_print("!!!WARNNING: {}: {}".format(time.strftime("%Y-%m-%d %H:%M:%S"), message))

    @staticmethod
    def error(message):
        """Logs message with a colored timestamp."""
        log_print("!!!ERROR: {}: {}".format(time.strftime("%Y-%m-%d %H:%M:%S"), message))
    ...
    def print_header(title):
        ...
        """
        log_print("".join("=" for _ in range(80)))
        log_print(title.upper())
        log_print("".join("=" for _ in range(80)))
    ...
    def debug(message, label=None):
        ...
        log_print("{}{}".format(prefix, message))
